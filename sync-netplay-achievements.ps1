# sync-netplay-achievements.ps1
# Usage:
#   .\sync-netplay-achievements.ps1
#   .\sync-netplay-achievements.ps1 -Force
...
function Assert-NoUnexpectedChanges {
    $statusLines = git status --short --ignore-submodules=dirty
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Merge conflict detected. Aborting merge." -ForegroundColor Red
    git merge --abort

    if ($LASTEXITCODE -ne 0) {
        Write-Host "Merge abort failed. Manual cleanup may be needed." -ForegroundColor Red
    }
...
Write-Host "Done. Xenia Netplay deployed to $DeployDestination." -ForegroundColor Green